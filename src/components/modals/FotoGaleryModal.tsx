...
const FotoGaleryModal: React.FC<ModalProps> = ({ isModalOpen, closeModal, IsVisible, url }) => {
	...
	return (
		<div className={`${isModalOpen && 'modal-overlay relative z-10'}`} onClick={handleOverlayClick}>
			{isModalOpen &&
				<div ref={modalRef} className={`absolute top-[107px] md:top-[50px] lg:top-[50px] right-[50%] translate-x-[50%] text-white bg-block-dark overflow: hidden; ${IsVisible && `modal-visible`}  ${!IsVisible && `modal-hidden`} rounded-sm`}>
					<div className='w-[336px] sm:w-[343px] md:w-[532px] lg:w-[980px] p-2 md:p-3  relative'>
						...
						<div className='w-full mx-auto'>
							<div className="w-full min-h-[250px] md:min-h-[350px]  lg:min-h-[680px] relative">
								<Image
									src={url}
									alt='Photo'
									fill
