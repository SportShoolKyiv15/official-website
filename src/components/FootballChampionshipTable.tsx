		...
		<table onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} ref={ref} className="border-collapse text-sm md:text-base">
			<thead>
				<tr className="flex md:table-row text-center h-[50px] text-black-text bg-table-title overflow-hidde">
					...
					<th className={`flex justify-center items-center md:table-cell min-w-[50px] sm:min-w-[52px] md:min-w-[51px] lg:w-[248px] border-r border-arrow bg-table-title ${extra}`}>Пор</th>
					<th className={`flex justify-center items-center md:table-cell min-w-[100px] sm:min-w-[103px] md:min-w-[51px] lg:w-[248px] border-r border-arrow bg-table-title ${extra}`}>Пер</th>
					<th className={`flex justify-center items-center md:table-cell min-w-[100px] sm:min-w-[103px] md:min-w-[84px] lg:w-[248px] bg-table-title ${extra}`}>Рахунок</th>
				</tr>
			</thead>
			<tbody>
				{comands.length &&
					comands.map((item, idx) => (
						<tr key={idx} className="flex md:table-row text-center">
							...
							<td className={`flex justify-center items-center md:table-cell min-w-[50px] sm:min-w-[52px] md:min-w-[60px] border-r border-b border-arrow ${extra}`}>{item.points}</td>
							<td className={`flex justify-center items-center md:table-cell min-w-[50px] sm:min-w-[51px] md:min-w-[45px] border-r border-b border-arrow ${extra}`}>{item.draw}</td>
							<td className={`flex justify-center items-center md:table-cell min-w-[50px] sm:min-w-[52px] md:min-w-[51px] border-r border-b border-arrow ${extra}`}>{item.defeat}</td>
							<td className={`flex justify-center items-center md:table-cell min-w-[100px] sm:min-w-[103px] md:min-w-[51px] border-r border-b border-arrow ${extra}`}>{item.win}</td>
							<td className={`flex justify-center items-center md:table-cell min-w-[100px] sm:min-w-[103px] md:min-w-[84px] border-r border-b border-arrow ${extra}`}>{item.score}</td>
						</tr>
					))
				}
			</tbody>
